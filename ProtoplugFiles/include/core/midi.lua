...
local Buffer_mt = {
	...
	__index = {
		...
		eachEvent = function(self)
			...
			return coroutine.wrap(function()
				...
				while msg < dataStart + nBytes do
					...
					coroutine.yield(ret)
					msg = msg + ffi.sizeof("int32_t") + ffi.sizeof("uint16_t") + ffi.sizeof("uint8_t") * ret.dataSize
				end
				self:unlock()
			end)
		end,
		...
		addEvent = function(self, event)
			self:checkLock()
			local nBytes = protolib.MidiBuffer_getDataSize(self)
			protolib.MidiBuffer_resizeData(
				self,
				nBytes + event.dataSize + ffi.sizeof("int32_t") + ffi.sizeof("uint16_t")
			)
			local dataStart = protolib.MidiBuffer_getDataPointer(self)
		...
		unlock = function(self)
			local addr = tonumber(ffi.cast("int", protolib.MidiBuffer_getDataPointer(self)))
			if not locks[addr] then
				return
			end
			locks[addr] = locks[addr] - 1
			if locks[addr] < 1 then
				locks[addr] = nil
			end
		end,
...
--- Constructor : Polyphonic Aftertouch.
-- @param channel (1-16)
-- @param number control number (0-247)
-- @param value control value (0-127)
-- @param[opt=0] pos sample offset
-- @treturn midi.Event
-- @function Event.aftertouch
midi.Event.aftertouch = function(channel, note, val, pos)
	pos = pos or 0
	local n = midi.Event(pos, 3)
...
local MidiEvent_mt = {
	...
	__index = {
		...
		setChannel = function(self, channel)
			if band(self.data[0], 0xf0) ~= 0xf0 then
				self.data[0] = bor(band(self.data[0], 0xf0), channel - 1)
			end
		end,
		...
		isNoteOff = function(self)
			return (
				band(self.data[0], 0xf0) == 0x80 or (band(self.data[0], 0xf0) == 0x90 and band(self.data[2], 0x7f) == 0)
			)
		end,
		...
		getPitchBendValue = function(self)
			if not self:isPitchBend() then
				error("not a pitch bend event")
			end
		...
		getPressure = function(self)
			if not self:isPressure() then
				error("not a pressure event")
			end
			return self.data[1]
		end,

		--- Is a Polyphonic Aftertouch event.
		-- @treturn boolean whether event is an aftertouch event.
