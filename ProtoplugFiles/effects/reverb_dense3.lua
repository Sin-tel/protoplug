...
local len3 = { 4229, 4391, 4721, 4621, 4871, 5297, 5323, 5741, 5897, 6421, 6701, 7069, 7103, 7669, 8039, 8353 }
...
for i = 1, n do
	delays[i] = Line(max_l)
	delays3[i] = Line(max_l3)
	fb_gains[i] = 0.5
	-- filters[i] = OnePole({ type = "hs", f = 2000, gain = -3 })
	filters[i] = OnePole({ type = "lp", f = 15000 })
	filters2[i] = OnePole({ type = "ls", f = 300, gain = -3 })
	-- len3[i] = math.random(max_l3) * 0.5 + 0.4
end
...
local lp1_l = Filter({ type = "lp", f = 15000, Q = 0.5412 })
local lp1_r = Filter({ type = "lp", f = 15000, Q = 0.5412 })
local lp2_l = Filter({ type = "lp", f = 15000, Q = 1.3066 })
local lp2_r = Filter({ type = "lp", f = 15000, Q = 1.3066 })

local function from_db(x)
	return 10 ^ (x / 20)
end

local function set_fb()
	for i = 1, n do
		local l = len3[i] * tmul3_

		local gain = (-60 * l) / (t60 * 44100)
		fb_gains[i] = from_db(gain)

		-- filters[i].update({ gain = gain * darkness })
		filters[i].update({ f = 15000 - 5000 * darkness })
		filters2[i].update({ gain = gain * 0.5 })
	end
end
...
function plugin.processBlock(samples, smax)
	for k = 0, smax do
		...
		tmul3 = tmul3 + (tmul3_ - tmul3) * 0.001
		...
		local in_l = lp1_l.process(input_l)
		in_l = lp2_l.process(in_l)
		local in_r = lp1_r.process(input_r)
		in_r = lp2_r.process(in_r)
		...
		for i = 1, n do
			delays[i].push(s[i])
			local l = sign_l[i] * delays[i].goBack_int((len2_l[i] - 1) * tmul2 + 1)
			local r = sign_r[i] * delays[i].goBack_int((len2_r[i] - 1) * tmul2 + 1)
		...
		for i = 1, n do
			local l = delays3[i].goBack_int(len3[i] * tmul3)
			l = filters[i].process(l)
			l = filters2[i].process(l)
			s2[i] = fb_gains[i] * l
		end
		s2 = hadamard(s2)

		for i = 1, n do
			delays3[i].push(s[i] + s2[i])
		end
		...
		local kl = 3 * s2[1]
		local kr = 3 * s2[2]
		sl = sl * (1 - mix_late) + kl * mix_late
		sr = sr * (1 - mix_late) + kr * mix_late
