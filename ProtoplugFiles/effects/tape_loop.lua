...
-- wow
local lfo_freq = 2 * math.pi * 0.4 / sample_rate
local lfo_mod = 0.1
...
function stereoFx.Channel:tick_tape(x)
	...
	while self.accum >= 1.0 do
		...
		local x_interp = lagrange4(x, self.x1, self.x2, self.x3, a)
	...
	local out = lagrange4(y0, y1, y2, y3, frac)
